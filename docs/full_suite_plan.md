...
The AMPETP pilot readiness report is generated at `outputs/ampetp_readiness_report.json` and `outputs/ampetp_readiness_report.md`. Current status is `paper_pilot_ready` with 8 checks passed and 0 failed.
...
The deterministic CCDC sensitivity generator is now generic via `scripts/build_ccdc_sensitivity_set.py`. It has been applied to ibuprofen CCDC 774097 as a neutral therapeutic contrast. The local MACE-only contrast summary is generated at `outputs/ibuprofen_sensitivity_summary_mace.json` and `outputs/ibuprofen_sensitivity_summary_mace.md`; AIMNet2 contrast remains a Docker/Linux follow-up.
